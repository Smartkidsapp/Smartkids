...
import { FilterPaymentDto } from './dtos/filter-payments.dto';
import { Payment } from './schemas/payment.schemas';

export class PaymentAggregationStagesBuilder {
  private listStages: PipelineStage[] = [];
  private countStages: PipelineStage[] = [];
  private filter: FilterQuery<Payment> = {
    $and: [],
  };

  constructor(
    private paginationDto: FilterPaymentDto & {
      limit?: number;
      page?: number;
      groupBy?: string;
    },
  ...
  reset(
    paginationDto?: FilterPaymentDto & {
      limit?: number;
      page?: number;
    },
  ) {
