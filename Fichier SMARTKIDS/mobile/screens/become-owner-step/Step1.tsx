import { Box, Button, HStack, Icon, Pressable, ScrollView, VStack } from '@gluestack-ui/themed';
...
const Step1 = (props: componentNameProps) => {
    ...
    const onSubmit = () => {
        if (createEtablissement?.category) {
            onChangeIndex();
        }
    };
                ...
                <Fragment>
                    ...
                    <HStack px={24} pt={16} pb={Platform.OS == 'android' ? 16 : 0} alignItems='center' justifyContent='space-between'>
                        ...
                        <Button
                            ...
                            px={50}
                            borderRadius={10}
                            bgColor={createEtablissement?.category ? '$primary' : '$light200'}
                            onPress={onSubmit}
                        >
                            <StyledText size={14} color={createEtablissement?.category ? '#000' : Colors.light.gray} weight='semi-bold'>{t('next')}</StyledText>
                        </Button>
                    </HStack>
                </Fragment>
